#!/usr/bin/env python3
"""
Defined _hash_password function
"""
import bcrypt
...
def _hash_password(password: str) -> bytes:
    """
    Hashed password string and returns it in bytes form
    Args:
...
def _generate_uuid() -> str:
    """
    Generated uuid and return its string representation
    """
    return str(uuid4())
...
class Auth:
    ...
    def register_user(self, email: str, password: str) -> User:
        """
        Registered new user and return a user object
        Args:
    ...
    def valid_login(self, email: str, password: str) -> bool:
        """
        Validated user's login credentials and return True if correct
        or False if they are not
        Args:
            email (str): user's email address
            password (str): user's password
        Return:
            True if credentials correct, else False
        """
        try:
    ...
    def create_session(self, email: str) -> Union[None, str]:
        """
        Create session_id for an existing user and update user's
    ...
    def get_user_from_session_id(self, session_id: str) -> Union[None, U]:
        """
        Takes session_id and returns corresponding user, if one exists,
        else returns None
        Args:
    ...
    def destroy_session(self, user_id: int) -> None:
        """
        Takes user_id and destroy user's session and update their
    ...
    def get_reset_password_token(self, email: str) -> str:
        """
        Generates reset_token uuid for user identified by given email
        Args:
            email (str): user's email address
        Return:
            newly generated reset_token for relevant user
        """
        try:
            user = self._db.find_user_by(email=email)
        except NoResultFound:
